...
import org.apache.spark.sql.rapids.aggregate.{
...
import org.apache.spark.sql.types.{ArrayType, DataType, IntegerType, LongType, MapType, StructType}
 ...
 *
 * Fusion applies when every partial aggregate is non-distinct, unfiltered, and one of the simple
 * libcudf hash aggregates: [[GpuSum]], [[GpuCount]], [[GpuBasicMin]], [[GpuBasicMax]].
 * [[org.apache.spark.sql.rapids.aggregate.GpuFloatMin]] /
 * [[org.apache.spark.sql.rapids.aggregate.GpuFloatMax]] (multi-column partial
 * state for NaN rules) and compound aggs (e.g. AVG) are not fused.
 *
 * Placement follows [[GpuTransitionOverrides]] (early, with other transition optimizations), not
 * the alfred fused-operator design.
 *
 * JNI rollup is **not** fused when any grouping key or aggregate input expression carries a nested
 * Spark type ([[ArrayType]], [[StructType]], [[MapType]]). Atomic types including
 * [[org.apache.spark.sql.types.StringType]] and [[org.apache.spark.sql.types.BinaryType]] are
 * allowed.
 *
 * **When not to fuse:** If [[GpuExpandExec.child]] is already ordered on every rollup data key
 * (ascending, same order as grouping), fusion is skipped. The plan stays Expand → partial
 * [[GpuHashAggregateExec]], which can use the existing single-pass / sorted-key libcudf path.
 *
 * **When fused:** [[GpuHashAggregateExec.allowSinglePassAgg]] and `forceSinglePassAgg` are cleared.
 * The partial iterator then always uses the hash path (`setSort(false)`), JNI rollup with
 * `keySorted = false`, and does not insert a sort for this node—appropriate when the child was
 * not already ordered on the rollup keys.
 */
class GpuJniRollupFusionRule(conf: RapidsConf) extends Rule[SparkPlan] with Logging {
  ...
  /** True for single-pass partial aggs that match libcudf hash rollup (no extra JNI work). */
  private def isSimpleAggregateForJniRollup(fn: GpuAggregateFunction): Boolean = fn match {
    case _: GpuSum => true
    case _: GpuCount => true
    case _: GpuBasicMin => true
    case _: GpuBasicMax => true
    case _ => false
  }

  private def aggregatesSupportedByJniRollup(agg: GpuHashAggregateExec): Boolean = {
    agg.aggregateExpressions.forall { e =>
      !e.isDistinct && e.filter.isEmpty && isSimpleAggregateForJniRollup(e.origAggregateFunction)
    }
  }

  private def isNestedSparkTypeForJniRollup(dt: DataType): Boolean = dt match {
    case _: ArrayType | _: StructType | _: MapType => true
    case _ => false
  }

  /** True if any subexpression carries a nested type in its [[Expression.dataType]]. */
  private def expressionTreeContainsNestedType(root: Expression): Boolean = {
    root.exists(e => isNestedSparkTypeForJniRollup(e.dataType))
  }
  ...
  private def tryFuse(expand: GpuExpandExec, agg: GpuHashAggregateExec): Option[GpuHashAggregateExec] = {
    ...
    if (!aggregatesSupportedByJniRollup(agg)) {
      logInfo("[GpuJniRollupFusion] Partial aggregates are not all supported simple types " +
        "(SUM/COUNT/MIN/MAX basic, non-distinct, no filter); skipping JNI rollup fusion")
      return None
    }
    ...
    // Pre-sorted on rollup keys: keep Expand + legacy partial agg (sorted/single-pass libcudf).
    if (newGrouping.nonEmpty) {
      val requiredKeyOrder = newGrouping.map(SortOrder(_, Ascending))
      if (SortOrder.orderingSatisfies(expand.child.outputOrdering, requiredKeyOrder)) {
        logInfo("[GpuJniRollupFusion] Expand child is ordered on rollup data keys; skipping fusion " +
          "so partial aggregate can use Expand + sorted-key path")
        return None
      }
    }

    if (newGrouping.exists(expressionTreeContainsNestedType) ||
        newAggs.exists(ae => expressionTreeContainsNestedType(ae.origAggregateFunction))) {
      logInfo("[GpuJniRollupFusion] Nested array/struct/map in grouping or aggregate inputs; " +
        "skipping JNI rollup fusion")
      return None
    }
