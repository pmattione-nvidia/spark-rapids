...
val GPU_COREDUMP_PIPE_PATTERN = conf("spark.rapids.gpu.coreDump.pipePattern")
  ...
  val ENABLE_JNI_ROLLUP_EXPAND_FUSION = conf("spark.rapids.sql.jniRollupExpandFusion.enabled")
    .doc("When true, fuses GpuExpandExec into GpuHashAggregateExec for Spark ROLLUP partial " +
      "aggregates and runs the rollup path via spark-rapids-jni (Rollup.aggregate), including " +
      "spark_grouping_id values aligned with Spark GpuExpand literals, when every partial " +
      "aggregate is a non-distinct SUM without filter. Fusion is skipped when Expand would run " +
      "its internal multi-tier pre-projection (see GpuExpandExec).")
    .internal()
    .booleanConf
    .createWithDefault(true)
