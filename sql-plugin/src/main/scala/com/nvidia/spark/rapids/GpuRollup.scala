...
/**
 * Fused ROLLUP partial aggregate via spark-rapids-jni (`spark_rapids_jni::rollup`).
 *
 * Output always includes `spark_grouping_id` after data keys (same contract as Ferdinand /
 * ExpandAggregate-style fused kernels). Non-fused plans use libcudf `Table.groupBy` in
